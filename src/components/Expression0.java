 ...
 * Key Features:
 * <ul>
 * <li>Uses Apfloat for calculation and result</li>
 ...
 *  
 *  Apfloat a = new Apfloat("2.4");
 *  Apfloat b = new Apfloat("9.235");
 ...
 * </tr>
 * </table>
 * *Boolean operators result always in a Apfloat value of 1 or 0 (zero). Any
 ...
 * `eval()` method will be called with the Apfloat values of the operands.
 ...
 * `eval()` method will be called with a list of the Apfloat parameters. All
 ...
 *         Apfloat sum = parameters.get(0).add(parameters.get(1)).add(parameters.get(2));
 *         return sum.divide(new Apfloat(3));
 *     }
 * });
 * 
 * e.eval(); // returns 16
...
//                        "2.71828182845904523536028747135266249775724709369995957496696762772407663");
	 ...
	 */
	private Map<String, Apfloat> variables = new TreeMap<String, Apfloat>(String.CASE_INSENSITIVE_ORDER);
	...
	/**
	 * The Apfloat representation of the left parenthesis, 
	...
	public abstract class Function extends LazyFunction {
		...
		public LazyNumber lazyEval(List<LazyNumber> lazyParams) {
			final List<Apfloat> params = new ArrayList<Apfloat>();
		 ...
		 *            {@link List} of {@link Apfloat} values.
		 * @return The function must return a new {@link Apfloat} value as a
		 *         computing result.
		 */
		public abstract Apfloat eval(List<Apfloat> parameters);
	}
	/**
	...
	public abstract class Operator {
		 ...
		 */
		public abstract Apfloat eval(Apfloat v1, Apfloat v2);
	}
	 ...
	 */
	public Expression0(String expression) {
	 ...
	 */
	public Expression0(String expression, MathContext defaultMathContext) {
...
//				v2 = v2.multiply(new Apfloat(signOf2)); // n2 is now positive
//				Apfloat remainderOf2 = v2.remainder(Apfloat.ONE);
//				Apfloat n2IntPart = v2.subtract(remainderOf2);
//				Apfloat intPow = v1.pow(n2IntPart.intValueExact(), mc);
//				Apfloat doublePow = new Apfloat(Math.pow(dn1,
//						remainderOf2.doubleValue()));
//
//				Apfloat result = intPow.multiply(doublePow, mc);
//				if (signOf2 == -1) {
//					result = Apfloat.ONE.divide(result, mc.getPrecision(),
...
//				return result;
		...
		addOperator(new Operator("&&", 4, false) {
			@Override
			public Apfloat eval(Apfloat v1, Apfloat v2) {
				boolean b1 = !v1.equals(Apfloat.ZERO);
				boolean b2 = !v2.equals(Apfloat.ZERO);
				return b1 && b2 ? Apfloat.ONE : Apfloat.ZERO;
			}
		});
		...
		addOperator(new Operator(">", 10, false) {
			@Override
			public Apfloat eval(Apfloat v1, Apfloat v2) {
				return v1.compareTo(v2) == 1 ? Apfloat.ONE : Apfloat.ZERO;
			}
		});
		...
		addOperator(new Operator(">=", 10, false) {
			@Override
			public Apfloat eval(Apfloat v1, Apfloat v2) {
				return v1.compareTo(v2) >= 0 ? Apfloat.ONE : Apfloat.ZERO;
			}
		});
		...
		addOperator(new Operator("<", 10, false) {
			@Override
			public Apfloat eval(Apfloat v1, Apfloat v2) {
				return v1.compareTo(v2) == -1 ? Apfloat.ONE
						: Apfloat.ZERO;
			}
		});
		...
		addFunction(new Function("NOT", 1) {
			@Override
			public Apfloat eval(List<Apfloat> parameters) {
				boolean zero = parameters.get(0).compareTo(Apfloat.ZERO) == 0;
				return zero ? Apfloat.ONE : Apfloat.ZERO;
			}
		});
		...
		addLazyFunction(new LazyFunction("IF", 3) {
			@Override
			public LazyNumber lazyEval(List<LazyNumber> lazyParams) {
				boolean isTrue = !lazyParams.get(0).eval().equals(Apfloat.ZERO);
...
//				return new Apfloat(d, mc);
                                return new Apfloat(String.valueOf(d));
			}
		});
...
//				return new Apfloat(d, mc);
		...
		addFunction(new Function("MAX", -1) {
			...
			public Apfloat eval(List<Apfloat> parameters) {
				...
				Apfloat max = null;
				for (Apfloat parameter : parameters) {
...
//				return new Apfloat(ix, mc.getPrecision());
		...
		variables.put("e", e);
		variables.put("PI", PI);
		variables.put("TRUE", Apfloat.ONE);
		variables.put("FALSE", Apfloat.ZERO);
...
//						return variables.get(token).round(mc);
				...
				stack.push(new LazyNumber() {
					public Apfloat eval() {
//						return new Apfloat(token, mc);
                                                return new Apfloat(token);
					}
				});
			}
		}
//		return stack.pop().eval().stripTrailingZeros();
	 ...
	 */
	public Expression0 setPrecision(int precision) {
	 ...
	 * @return The expression, allows to chain methods.
	 */
	public Expression0 setRoundingMode(RoundingMode roundingMode) {
	 ...
	 * @return The expression, allows to chain methods.
	 */
	public Expression0 setVariable(String variable, Apfloat value) {
	 ...
	 * @return The expression, allows to chain methods.
	 */
	public Expression0 setVariable(String variable, String value) {
		if (isNumber(value))
			variables.put(variable, new Apfloat(value));
		else {
	 ...
	 * @return The expression, allows to chain methods.
	 */
	public Expression0 with(String variable, Apfloat value) {
	 ...
	 * @return The expression, allows to chain methods.
	 */
	public Expression0 and(String variable, String value) {
		return setVariable(variable, value);
	}
	 ...
	 * @return The expression, allows to chain methods.
	 */
	public Expression0 and(String variable, Apfloat value) {
		return setVariable(variable, value);
	}
	 ...
	 * @return The expression, allows to chain methods.
	 */
	public Expression0 with(String variable, String value) {
		return setVariable(variable, value);
	}
